...
async def ai(message: Message, state: FSMContext):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            await message.answer(f"Вот твоя картинка, по запросу: {user_message}: {image_url}")  # Используем f-string для подстановки URL
        else:
            await message.answer("Ошибка при генерации изображения!")  # Выводим текст ошибки из ответа
    
    except Exception as e:
        await message.answer("Ошибка: " + str(e))  # Ловим любые исключения
